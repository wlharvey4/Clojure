...
for Clojure, available in the book’s resources at
...
- Go to the [[https://github.com/flyingmachine/emacs-for-clojure][Emacs configuration files on GitHub]] (includes instructions)
...
option is to delete the current configuration and install these in their
place.  Another option is to save the current configuration and install these
temporarily, and then be able to switch between them.  A third option is to
merge the current configuration with this configuration instead of completely
deleting it.

For the time being, I will use the second option: keep both and provide a means
of toggling between them.  These files will go into a directory
~/.emacs.d-cfbt/~.  The current ~/.emacs.d~ will be moved into
~/.emacs.d-original~.  A sym-link will be set up to use either
~/.emacs.d-original~ or ~/.emacs-d-cfbt~.  Emacs should be restarted when a
switch is made.
...
*** Switching Between Configuration Setups

#+name:cfbt-toggle-init-files
#+header: :mkdirp yes
#+header: :tangle tools/toggle-init
#+header: :shebang "#!/usr/bin/env bash"
#+begin_src sh
  read -N 1 -ep "Toggle ~/.emacs.d ? "
  echo REPLY is $REPLY
  [[ $REPLY =~ "y" ]] || {
      echo "Aborting toggle procedure."
      exit 0
  }
  ...
  then
      if [[ -h ~/.emacs.d ]]
      then
	  if [[ $(readlink ~/.emacs.d) == ".emacs.d-original" ]]
	  then
	      ln -vsF ".emacs.d-cfbt" ~/.emacs.d
	  else
	      ln -vsF ".emacs.d-original" ~/.emacs.d
	  fi

      else
	  cp -vR ~/.emacs.d ~/.emacs.d-original
	  rm -Rf ~/.emacs.d
	  ln -vs ~/.emacs.d-original ~/.emacs.d
      fi
  else
      if [[ -d ~/.emacs.d-original ]]
      then
	  ln -vsF ~/.emacs.d-original ~/.emacs.d
      else
	  if [[ -d ~/.emacs.d-cfbt ]]
	  then
	      ...
	      echo "Neither ~/.emacs.d nor ~/.emacs.d-original nor ~/.emacs.d-cfbt exists."
	      echo "Aborting toggle procedure."
	  fi
      fi
  fi
